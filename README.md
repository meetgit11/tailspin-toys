...
- **Catalog filtering** — the home page includes category and publisher filters so visitors can narrow the featured game list without leaving the static site.
...
The shared **Issue Triage Board** canvas (`.github/extensions/issue-triage-board/`) highlights the three backlog issues most likely to need attention and provides buttons to add issues to the current session context. It runs as a local Copilot extension and does not change the application runtime.
